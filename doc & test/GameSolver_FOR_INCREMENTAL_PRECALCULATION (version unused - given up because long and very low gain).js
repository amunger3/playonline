...
try {
  ...
  let baseOfMaxPerformanceEvaluationTime = 30000000000000000; // 30 seconds / much higher in (precalculation mode => see GameSolver_FOR_INCREMENTAL_PRECALCULATION.js)
  let maxPerformanceEvaluationTime = -1;

  let refNbOfCodesForSystematicEvaluation = XXXX; // (high values may induce latencies) precalculation mode: TBC
  let refNbOfCodesForSystematicEvaluation_AllCodesEvaluated = SAMEXXXX; // (shall be <= refNbOfCodesForSystematicEvaluation - high values may induce latencies) precalculation mode: TBC
  ...
  let precalculated_games_5columns =
    XXX; // (precalculation mode: TBC with depth-2 or depth-3 precalculations)
  ...
  function evaluatePerformances(depth, listOfCodes, nbCodes, particularCode, areCurrentGameOrCodePrecalculated_p, nbOfClassesFirstCall_p) {
    ...
    if (depth == -1) { // first call
      ...
      res = recursiveEvaluatePerformances(depth, listOfCodes, nbCodes,  true, false /* (precalculation mode) */);
  ...
  function recursiveEvaluatePerformances(depth, listOfCodes, nbCodes, possibleGame, precalc_depth3 /* (precalculation mode) */) {
    ...
    let depth2or3 = 2 or 3 XXX; // (precalculation mode: TBC)
                                ...
                                && ( (next_cur_game_idx <= 1)
                                     ...
                                     || ( (depth2or3 == 3) // (new possible depth-3 criterion, skipped when depth-2 is considered)
                                          ...
                                          && ( (possibleGame && (codeHandler.nbDifferentColors(curGame[0]) <= 2) && (codeHandler.nbDifferentColors(curGame[1]) <= 2) && (codeHandler.nbDifferentColors(curGame[2]) <= 2))
                                               ...
                                               || ((codeHandler.nbDifferentColors(curGame[0]) > 2) && codeHandler.isVerySimple(curGame[1]) && (codeHandler.nbDifferentColors(curGame[2]) == 1)) // only 1-color codes at 3rd attempt to minimize depth-3 list size
                                               || ((codeHandler.nbDifferentColors(curGame[1]) > 2) && codeHandler.isVerySimple(curGame[0]) && (codeHandler.nbDifferentColors(curGame[2]) == 1)) // only 1-color codes at 3rd attempt to minimize depth-3 list size
                                               || precalc_depth3 )
                                        )
                                   )
    ...
    for (idx1 = 0; idx1 < nbCodesToGoThrough; idx1++) { // (precalculation mode)
      ...
      // 83: 11234
      // 668: 12345
      if (first_call && (idx1 != XXX)) { // precalculation mode: TBC
        continue;
      }
      if (idx1 < nbCodes) { // possible codes
        cur_code = listOfCodes[idx1];
      }
      else { // impossible codes
        cur_code = initialCodeListForPrecalculatedMode[idx1 - nbCodes]; // (precalculation mode)
        ...
        for (let i = 0; i < next_cur_game_idx; i++) {
          ...
          if (precalculation_mode_mark.nbBlacks + precalculation_mode_mark.nbWhites < 5) {
            five_pegs = false;
          }
        ...
        if (!skip_cur_code) {
          if ((next_cur_game_idx == 2) && (codeHandler.nbDifferentColors(curGame[0]) > 2) && (codeHandler.nbDifferentColors(curGame[1]) > 2)) {
            // Precalculation optimization (2/4): skip impossible codes for complex games
            let skip_code_complex_games = true;
            if ((nbCodes >= 700) && (codeHandler.nbDifferentColors(cur_code) == 1)) { // a few classical very inefficient impossible codes
              skip_code_complex_games = false; // classical way of playing
              // concerns a high number of games => extended to depth3 only in some particular cases (see above in this function)
            }
            if ((nbCodes >= 700) && (precalculation_mode_mark_first_2_codes_at_depth2.nbBlacks >= 4)) { // classical very inefficient ways of playing
              if (four_blacks_2) { // four_blacks_2 instead of four_blacks here to minimize depth-3 list size + decrease precalculation time
                skip_code_complex_games = false;
                new_precalc_depth3 = true; // few codes => always extended to depth3
              }
            }
            if ( (nbCodes >= 700) && (precalculation_mode_mark_first_2_codes_at_depth2.nbBlacks + precalculation_mode_mark_first_2_codes_at_depth2.nbWhites >= 5) && (codeHandler.nbDifferentColors(curGame[0]) >= 5) // classical very inefficient ways of playing
                 && (nbCodes <= 1900) // higher numbers of possible codes increase precalculation time "too much" (by weeks!)
               ) {
              if (five_pegs) { // extra conditions, like same-color-sharing ones, increase precalculation time "too much"
                skip_code_complex_games = false;
                // many codes => not extended to depth3
              }
            }
            skip_cur_code = skip_code_complex_games;
          }
          else {
            // Precalculation optimization (3/4): skip impossible cur code if acceptable (a few classical impossible codes are kept for better coverage)
        ...
        for (mark_idx = nbMaxMarks-1; mark_idx >= 0; mark_idx--) {
          ...
          if (nextNbCodes > 0) {
            ...
            if (mark_idx == best_mark_idx) {
              ...
              sum = sum + nextNbCodes * recursiveEvaluatePerformances(next_depth, nextListsOfCodes[mark_idx], nextNbCodes, ((idx1 < nbCodes) && possibleGame), new_precalc_depth3 /* (precalculation mode) */); // (Note: possibleGame = ((idx1 < nbCodes) && possibleGame))
              if (sum_marks == nbCodes) break;
    ...
    if (precalculation_mode) { // (precalculation mode)
      ...
      else if (precalculation_time >= 2000*XXX) { // 2000 = 2.0 seconds on i5 processor or on Linux VB running on i7 processor // precalculation mode: TBC
        ...
        for (mark_idx = nbMaxMarks-1; mark_idx >= 0; mark_idx--) {
          ...
          if (nextNbCodes > 0) {
            ...
            if (mark_idx == best_mark_idx) {
              ...
              sum = sum + nextNbCodes * recursiveEvaluatePerformances(next_depth, nextListsOfCodes[mark_idx], nextNbCodes, false, false /* (precalculation mode) */);
  ...
  function handleMessage(data) {
    ...
    if (data.smm_req_type == 'INIT') {
      ...
      switch (nbColumns) {
        ...
        case 5:
          ...
          nbOfCodesForSystematicEvaluation = initialNbPossibleCodes; // (precalculation mode)
          nbOfCodesForSystematicEvaluation_AllCodesEvaluated = initialNbPossibleCodes; // (precalculation mode)
          nbOfCodesForSystematicEvaluation_ForMemAlloc = initialNbPossibleCodes; // game precalculation (*)
    ...
    // ***********
      ...
      if (data.precalculated_games != "") { // (duplicated code - begin)
        ...
        if (precalculated_games_5columns.length + data.precalculated_games.length > 70000000) { // 70 MB

...
try {
  ...
  function recursiveEvaluatePerformances(depth, listOfCodes, nbCodes, possibleGame /* (precalculation mode) */) {
    ...
    let precalculation_mode = ( (nbCodes >= minNbCodesForPrecalculation) // (**) only games for which there may not be enough CPU capacity / time to calculate performances online
                                ...
                                && ( (next_cur_game_idx <= 1)
                                     || (next_cur_game_idx == 2)
                                     || (false /* (this new depth-3 version is skipped as very long - previous results are kept) */
    ...
    for (idx1 = 0; idx1 < nbCodes; idx1++) {
      ...
      // if (depth <= 1) {
        ...
        if (areCurrentGameOrCodePrecalculated >= 0) { // both game and code were precalculated OR only game was precalculated (Note: "> 0" would not be good here, as a subsequent impossible code evaluation would fail instantly)
          appliedMaxPerformanceEvaluationTime = appliedMaxPerformanceEvaluationTime * 1.25; // it's a pity not to use precalculated results! (especially at 3rd attempt)
        }
    ...
    if (data.smm_req_type == 'INIT') {
      ...
      switch (nbColumns) {
        ...
        case 5:
          ...
          nbOfCodesForSystematicEvaluation = initialNbPossibleCodes; // in (precalculation mode)
          nbOfCodesForSystematicEvaluation_AllCodesEvaluated = initialNbPossibleCodes; // in (precalculation mode)
          nbOfCodesForSystematicEvaluation_ForMemAlloc = initialNbPossibleCodes; // game precalculation (*)

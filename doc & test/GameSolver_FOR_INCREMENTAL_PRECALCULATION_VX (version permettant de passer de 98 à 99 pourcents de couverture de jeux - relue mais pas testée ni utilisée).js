...
class CodeHandler {
  ...
  shareSameSetOfColors(code1, code2) {
    this.different_colors.fill(0);
    this.different_colors_bis.fill(0);
    for (let col = 0; col < this.nbColumns; col++) {
      let color1 = this.getColor(code1, col+1);
      this.different_colors[color1]++;
      let color2 = this.getColor(code2, col+1);
      this.different_colors_bis[color2]++;
    }
    for (let color = 1; color <= this.nbColors; color++) {
      if ((this.different_colors[color] > 0) && (this.different_colors_bis[color] == 0)) {
        return false;
      }
      if ((this.different_colors_bis[color] > 0) && (this.different_colors[color] == 0)) {
        return false;
      }
    }
    return true;
                                ...
                                && ( (next_cur_game_idx <= 1)
                                     ...
                                     || ( (depth2or3 == 3) // (new possible depth-3 criterion, skipped when depth-2 is considered)
                                          ...
                                          && ( (possibleGame && (codeHandler.nbDifferentColors(curGame[0]) <= 2) && (codeHandler.nbDifferentColors(curGame[1]) <= 2) && (codeHandler.nbDifferentColors(curGame[2]) <= 2))
                                               ...
                                               || ((codeHandler.nbDifferentColors(curGame[0]) > 2) && codeHandler.isVerySimple(curGame[1]) && (codeHandler.nbDifferentColors(curGame[2]) == 1))
                                               || ((codeHandler.nbDifferentColors(curGame[0]) > 2) && codeHandler.isVerySimple(curGame[2]) && (codeHandler.nbDifferentColors(curGame[1]) == 1))
                                               || precalc_depth3 )
                                        )
                                   )
    ...
    for (idx1 = 0; idx1 < nbCodesToGoThrough; idx1++) { // (precalculation mode)
      ...
      if (idx1 < nbCodes) { // possible codes
        ...
        if ((next_cur_game_idx == 2) && (codeHandler.nbDifferentColors(curGame[0]) > 2) && (codeHandler.nbDifferentColors(curGame[1]) > 2)) {
          ...
          if (precalculation_mode_mark_first_2_codes_at_depth2.nbBlacks >= 4) { // classical very inefficient ways of playing
            if (four_blacks) {
              skip_code_complex_games = false;
              new_precalc_depth3 = true; // few codes => always extended to depth3
            }
            else if ((nbCodes >= 700) && (codeHandler.nbDifferentColors(cur_code) <= 2)) {
              skip_code_complex_games = false;
              // many codes => not extended to depth3
            }
          }
          if (precalculation_mode_mark_first_2_codes_at_depth2.nbBlacks + precalculation_mode_mark_first_2_codes_at_depth2.nbWhites >= 5) { // classical very inefficient ways of playing
            if (codeHandler.shareSameSetOfColors(curGame[0], cur_code)) {
              skip_code_complex_games = false;
              // many codes => not extended to depth3
            }
            else if ((nbCodes >= 700) && (codeHandler.nbDifferentColors(cur_code) <= 2)) {

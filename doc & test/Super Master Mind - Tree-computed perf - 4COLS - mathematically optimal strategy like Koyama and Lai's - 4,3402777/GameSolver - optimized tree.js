...
try {
  ...
  let baseOfMaxPerformanceEvaluationTime = 3000000000000000000000; // 30 seconds / much higher in (precalculation mode)
  let maxPerformanceEvaluationTime = -1;

  let refNbOfCodesForSystematicEvaluation = 32768; // (high values may induce latencies) (***)
  let refNbOfCodesForSystematicEvaluation_AllCodesEvaluated = 32768; // (shall be <= refNbOfCodesForSystematicEvaluation - high values may induce latencies) (***)
 ...
 // *************************************************************************
  ...
  function recursiveEvaluatePerformances(depth, listOfCodes, nbCodes /*, possibleGame (precalculation mode) */) {
    ...
    /* for (idx1 = 0; idx1 < nbCodes; idx1++) {
      ...
      if (compute_sum) { // compute_sum
        ...
        if (nbCodesToGoThrough > nbCodes) { // There are some impossible codes
          for (mark_idx = nbMaxMarks-1; mark_idx >= 0; mark_idx--) {
            let nextNbCodes = nextNbsCodes[mark_idx];
            if (nextNbCodes > 0) {
      ...
      if (sum < best_sum) {
        ...
        if ((idx1 < nbCodes) && (average_group_size != 0)) {
          best_average_group_size_possible_codes = average_group_size * 1.04; // (with extra margin: works from 1.04 for MasterMind games)
        }
      }
  ...
  function handleMessage(data) {
    ...
    if (data.smm_req_type == 'INIT') {
      ...
      if (6*6*6*6 != fillShortInitialPossibleCodesTable(initialCodeListForPrecalculatedMode, nbOfCodesForSystematicEvaluation_ForMemAlloc)) { // (precalculation mode)
        ...
        if ( (precalculated_cur_game_or_code > 0) // both game and code were precalculated
          ...
          if (precalculated_cur_game_or_code > 0) { // both game and code were precalculated
            ...
            if (!performanceListsInitDoneForPrecalculatedGames) {
              ...
              arraySizeAtInit = initialNbPossibleCodes; // Math.ceil((3*previousNbOfPossibleCodes + nbOfCodesForSystematicEvaluation_ForMemAlloc)/4); // (overestimated for low values of previousNbOfPossibleCodes to ensure proper subsequent mem_reduc_factor application)
              listOfGlobalPerformances = new Array(arraySizeAtInit);
              maxDepthApplied = maxDepth; // "one-recursive-depth computing of performances" for current game and code (whether possible or impossible) => memory optimization
          ...
          // ***** Second evaluation phase in a game *****
            ...
            if (!performanceListsInitDone) {
              ...
              arraySizeAtInit = initialNbPossibleCodes; // Math.ceil((3*previousNbOfPossibleCodes + nbOfCodesForSystematicEvaluation)/4); // (overestimated for low values of previousNbOfPossibleCodes to ensure proper subsequent mem_reduc_factor application)
              listOfGlobalPerformances = new Array(arraySizeAtInit);
          ...
          if (best_global_performance != PerformanceUNKNOWN) { // performance evaluation succeeded
            ...
            throw new Error("UNKNOWN PERF 1!");
            best_global_performance = PerformanceUNKNOWN;
          ...
          throw new Error("UNKNOWN PERF 2!");
          best_global_performance = PerformanceUNKNOWN;
          code_played_relative_perf = PerformanceUNKNOWN;
          relative_perf_evaluation_done = false;
        }
